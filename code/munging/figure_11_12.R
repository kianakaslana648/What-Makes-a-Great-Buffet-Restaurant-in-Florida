...
regions_large3<-c("American","Chinese","Japanese", "Indian", "Latin","Halal","Brazilian")
...
df_character<-read.csv("yelp_dataset/binary_characteristic.csv")

regions_radar<-df_regions2 |> inner_join(df_scores,by="business_id") |> 
  select(business_id,name.x,categories,review_count,stars,Food.Score,Service.Score,Environment.Score,Variety.Score,Music.Score,Drink.Score,Sauce.Score,) |> rename(name=name.x) 
...
regions_radar<-regions_radar %>% 
  mutate_at(vars(Service.Score, Environment.Score,Variety.Score,Drink.Score,Sauce.Score), ~replace_na(.,mean(., na.rm = TRUE))) |> select(-Music.Score)
...
regions_score[regions_score$category!="Others",]|> select(category,Counts) |> rename(Value=Counts,Category=category) |> 
  write.csv("regions_category_distribution.csv",row.names = FALSE)

regions_score<-regions_score[regions_score$category!="Others",] |> pivot_longer(cols=c(Food,Service,Environment, Variety, Drink,Sauce),names_to="aspect")
regions_score
x <- toJSON(regions_score)

write(x, "regions.json")
