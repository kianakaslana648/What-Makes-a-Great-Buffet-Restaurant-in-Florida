(function (factory) {
  if (typeof define === "function" && define.amd) {
    ...
    if (d3 && d3.tip) d3.exploding_boxplot = factory(d3, d3.tip);
    else d3.exploding_boxplot = factory(d3);
  }
})(function (d3, d3tip) {
  var default_colors = [
    "#a6cee3",
  ...
  default_colors = ["#b2df8a", "#b2df8a", "#b2df8a", "#b2df8a"];
  var compute_boxplot = function (data, iqr_k, value) {
    ...
    var box_data = d3
      ...
      .key(function (d) {
        ...
        var type =
          d < quartiles[0] - iqr || d > quartiles[2] + iqr
            ? "outlier"
            : "normal";
  ...
  var exploding_boxplot = function (data, aes) {
    ...
    var width = 160;
    var boxpadding = 0.2;
    var margin = { top: 25, bottom: 40, left: 50, right: 10 };
    ...
    if (aes.group) {
      ...
      if (groups[0].key == "TRUE") {
        groups.splice(0, 0, groups.pop());
      }
      if (groups[0].key == "$$") {
        groups.splice(0, 0, groups.pop());
      }
    ...
    var _tipFunction = function (d) {
      return (
        ' <span style="color:' +
        colorscale(functorkey(aes.color)(d)) +
        '">' +
        functorkey(aes.label)(d) +
        '</span><span style="color:#DDDDDD;" > : ' +
        tickFormat(functorkey(aes.y)(d)) +
        "</span>"
      );
    };
    ...
    var chart = function (elem) {
      ...
      container
        ...
        .append("text")
        .attr("transform", "rotate(-90)")
        .attr("x", -margin.top - d3.mean(yscale.range()) + 25)
        .attr("dy", ".71em")
